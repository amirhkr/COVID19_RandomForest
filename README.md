...
- **Locally (with Conda):** Ensure [conda](https://docs.conda.io/en/latest/miniconda.html) is installed, then run `conda env create -f env.yml .` followed by `conda activate COVID19_RandomForest` to activate environment. Then add xgboost package by running conda `install -c anaconda py-xgboost`. Finally add SHAP package by running `pip install shap` . Then `jupyter lab` to instantiate notebook IDE.
