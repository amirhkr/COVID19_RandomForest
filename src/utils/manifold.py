...
def plot_manifolds(manifolds, titles, labels=None, savefig=False):
    """
    auth:ajm
    ...
    if labels is not None:
        fig,ax = plt.subplots(len(labels.columns),len(titles),figsize=[12,8])
        for i,l in enumerate(labels.columns):
            for j,t in enumerate(titles):
                m = manifolds[i].reset_index(drop=True)
                mask = ~(labels[l].isna())
#                 c = labels[l].loc[mask].reset_index(drop=True)
#                 c = 1 - (c - c.min())/(c.max()-c.min())
                c = labels[l]
                print(m.shape)
                _=ax.scatter(m.values[:,0],m.values[:,1],alpha=1,s=5,c=c,label=labels,cmap=cmap,norm=colors.DivergingNorm(vmin=c.min(), vcenter=c.median(), vmax=c.max()))
                ax.set_title(t)
                ax.set_xlabel('PC1')
                ax.set_ylabel('PC2')
                legend1 = ax.legend(*_.legend_elements(),
...
def summary(df, exps=['PCA'], labels=None):
    """
    """
    ...
    plot_manifolds(manifolds, algos.keys(), labels=labels, savefig=False)
    return df
